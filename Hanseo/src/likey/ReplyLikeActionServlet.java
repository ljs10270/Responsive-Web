...
@WebServlet("/ReplyLikeActionServlet")
public class ReplyLikeActionServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = replyLikeyDAO.replyLike(userID, boardID, replyGroup, getClientIP(request));
		
		if(result == 1) { //성공적으로 DB에 IP 삽입이 되었으면
			result = replyDAO.replyLike(boardID, replyGroup); //추천 수 증가
			
			if(result == 1) {
				session.setAttribute("messageType", "성공 메시지");
				session.setAttribute("messageContent", "해당 댓글을 추천 하였습니다.");
			...
			session.setAttribute("messageType", "오류 메시지");
			session.setAttribute("messageContent", "해당 댓글을 이미 추천 하셨습니다.");
			response.sendRedirect("boardShow.jsp?boardID=" + boardID);
			return;
		}
	}
